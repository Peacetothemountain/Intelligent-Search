...
class SecureSettingsRepository(
    ...
    fun retrieveSensitiveData(tokenKey: String): String? {
        ...
        return try {
            ...
            Log.e(TAG, "Decryption failed for key $tokenKey: cryptographic verification or tag mismatch", e)
            null
        } catch (e: Exception) {
            Log.e(TAG, "Failed to retrieve sensitive data for key $tokenKey", e)
            null
        }
    }

    fun hasSensitiveData(tokenKey: String): Boolean {
        return sharedPreferences.contains("${tokenKey}_data") && sharedPreferences.contains("${tokenKey}_iv")
    }
